...
export default function InstitutionStudentsPage() {
  ...
  useEffect(() => {
    fetch("/api/institution/students").then((r) => r.json()).then((d) => { setStudents(d.students || []); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  return (
    <InstitutionShell>
      <div className="mb-8">
        ...
        <h1 className="display text-3xl">Enrolled students.</h1>
      </div>
      <div className="border border-ink/10 bg-bone p-4 mb-6">
        <p className="text-xs opacity-40">Only names, emails, and course progress are shown. Personal safety plans, consultation details, and private notes are never visible to institution administrators.</p>
      </div>
