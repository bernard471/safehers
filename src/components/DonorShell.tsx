...
import { LayoutDashboard, Users, BarChart, FileText, LogOut, Menu, X } from "lucide-react";

const navItems = [
  { href: "/donor", label: "Dashboard", icon: LayoutDashboard },
  { href: "/donor/sponsored-cohorts", label: "Sponsored Cohorts", icon: Users },
  { href: "/donor/impact", label: "Impact Data", icon: BarChart },
  { href: "/donor/reports", label: "Reports", icon: FileText },
];

export default function DonorShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && session?.user?.role !== "donor" && session?.user?.role !== "admin" && session?.user?.role !== "super_admin") {
    ...
    <div className="min-h-screen bg-bone">
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-ink text-cream px-4 py-3 flex items-center justify-between">
        <Link href="/donor" className="flex items-center gap-2">
          ...
          <span className="display text-base">Donor Portal</span>
        </Link>
      ...
      <aside className={`fixed top-0 left-0 h-full z-50 w-64 bg-ink text-cream flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
        <div className="p-6 border-b border-cream/10 hidden lg:block">
          <Link href="/donor" className="flex items-center gap-2">
            ...
            <span className="display text-lg">Donor Portal</span>
          </Link>
        </div>
        ...
        <nav className="flex-1 py-4">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-6 py-3 text-sm transition-colors ${active ? "bg-cream/10 text-gold border-r-2 border-gold" : "text-cream/60 hover:text-cream hover:bg-cream/5"}`}>
        ...
        <div className="p-6 border-t border-cream/10">
          <p className="text-xs opacity-40 mb-3 truncate">{session.user?.name}</p>
      ...
      <main className="min-h-screen lg:ml-64 pt-14 lg:pt-0"><div className="p-4 sm:p-6 lg:p-12">{children}</div></main>
    </div>
  );
}
