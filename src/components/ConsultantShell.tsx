...
const navItems = [
  ...
  { href: "/consultant/bookings", label: "Bookings", icon: Calendar },
  { href: "/consultant/clients", label: "Clients", icon: Users },
  { href: "/consultant/reports", label: "Reports", icon: FileText },
];

export default function ConsultantShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && session?.user?.role !== "consultant" && session?.user?.role !== "admin" && session?.user?.role !== "super_admin") {
    ...
    <div className="min-h-screen bg-bone">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-ink text-cream px-4 py-3 flex items-center justify-between">
        <Link href="/consultant" className="flex items-center gap-2">
          ...
          <span className="display text-base">Consultant</span>
        </Link>
      ...
      <aside className={`fixed top-0 left-0 h-full z-50 w-64 bg-ink text-cream flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
        <div className="p-6 border-b border-cream/10 hidden lg:block">
          <Link href="/consultant" className="flex items-center gap-2">
            ...
            <span className="display text-lg">Consultant</span>
          </Link>
        </div>
